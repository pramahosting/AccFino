...
def create_checkout(body: dict = Body(...)):
    ...
    if plan["price_monthly"] == 0:
        raise HTTPException(400, "Demo plan is free — no payment needed")

    price_key = f"stripe_price_{billing_period}"
    price_id  = plan.get(price_key, "")
    if not price_id or price_id.startswith("price_REPLACE"):
        raise HTTPException(503, f"Stripe price not configured for {plan_id}/{billing_period}. "
                                  "Add price IDs to payments.py PLANS dict.")

    stripe     = _get_stripe()
    app_url    = os.environ.get("APP_URL", "http://localhost:3000").rstrip("/")

    # Accept optional custom modules list
    modules = body.get("modules", plan["modules"])
    ...
    session = stripe.checkout.Session.create(
        ...
        success_url=f"{app_url}/upgrade?success=1&plan={plan_id}",
...
def _activate_plan(db, user_id: int, plan_id: str, billing_period: str,
    ...
    lic.start_date         = str(today)
    lic.end_date           = str(end_date)
    lic.stripe_customer_id = stripe_customer_id
    lic.stripe_sub_id      = stripe_sub_id
    lic.amount_paid        = amount
    lic.modules            = json.dumps(custom_modules if custom_modules else plan["modules"])
    lic.notes              = f"Paid via Stripe — {plan['name']} {billing_period}"
    db.commit()
