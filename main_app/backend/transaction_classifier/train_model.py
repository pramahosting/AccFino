...
def train_from_df(df: pd.DataFrame, model_dir: Path = DEFAULT_MODEL_DIR) -> dict:
    ...
    if len(X) < 4:
        raise ValueError(
            f"Only {len(X)} usable row(s) after removing blanks -- need at least a handful "
            "of labeled examples per category to train anything meaningful."
        )

    # sklearn's stratify=y_cat requires EVERY class to have at least 2
    # members (so at least 1 can go to train and 1 to test) -- otherwise
    # it raises a ValueError and training fails outright. This is a real,
    # common failure: even this module's own downloadable sample CSV has
    # categories that appear only once, so stratifying unconditionally
    # made training crash on the very file the app hands out for testing.
    # Fall back to a non-stratified split instead of hard-failing when any
    # category is too rare to stratify on -- an imperfectly-balanced test
    # split is far better than training refusing to run at all.
    class_counts = y_cat.value_counts()
    can_stratify = class_counts.min() >= 2 if len(class_counts) > 1 else False
    rare_classes = sorted(class_counts[class_counts < 2].index.tolist()) if not can_stratify else []
    ...
    result = {
        "total_rows": len(X),
        "rows_used": len(X),  # alias -- react_api.py's /ml/train reads this key name
    ...
    if rare_classes:
        result["warning"] = (
            f"These categories had only 1 example each, so the test split wasn't stratified "
            f"and results may be less reliable: {', '.join(rare_classes)}. Add more examples "
            f"of these categories for a more trustworthy accuracy score."
        )
    return result
