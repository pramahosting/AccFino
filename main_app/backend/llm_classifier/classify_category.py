...
def load_disk_cache() -> Dict[str, Dict[str, str]]:
    """Loads the classification cache from Postgres (classifier_cache
    table) -- replaces ollama_cache.json. Returns the same
    {cache_key: {"category":..., "gst_category":...}} shape as before."""
    from db_app.database import SessionLocal
    from db_app.models.app_data import ClassifierCache
    db = SessionLocal()
    try:
        rows = db.query(ClassifierCache).all()
        return {r.cache_key: {"category": r.category or "", "gst_category": r.gst_category or ""} for r in rows}
    except Exception:
        return {}
    finally:
        db.close()
...
def save_disk_cache(cache: Dict[str, Dict[str, str]]) -> None:
    """Upserts the full in-memory cache dict back to Postgres. Called
    once at the end of a batch run (same call pattern as the old
    write-whole-file version), so a full replace-by-key here is fine."""
    from db_app.database import SessionLocal
    from db_app.models.app_data import ClassifierCache
    db = SessionLocal()
    try:
        for key, val in cache.items():
            row = db.get(ClassifierCache, key)
            if row:
                row.category = val.get("category")
                row.gst_category = val.get("gst_category")
            else:
                db.add(ClassifierCache(cache_key=key, category=val.get("category"), gst_category=val.get("gst_category")))
        db.commit()
    finally:
        db.close()
...
# -------------------------
# Groq call (via shared key pool -- see main_app/backend/utils/groq_pool.py)
# Replaces the old Ollama-only path. base_url/temperature/top_p kept in the
# signature for backward compatibility with existing callers, but base_url
# is now ignored (Groq's endpoint is fixed) and temperature/top_p are still
# honored.
# -------------------------
GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
GROQ_DEFAULT_MODEL = "openai/gpt-oss-20b"

def _groq_chat(system_msg: str, prompt: str, temperature: float, top_p: float) -> str:
    from db_app.database import SessionLocal
        ...
        for _ in range(3):
            ...
            if not resolved["groq_key"]:
                raise RuntimeError(
                    ...
                    "/groq-pool admin API or the Groq Keys tab."
                )
            model = resolved["model"] or GROQ_DEFAULT_MODEL
            payload = {
                "model": model,
            ...
            try:
                r = requests.post(
                    GROQ_API_URL, json=payload, timeout=60,
                    headers={"Content-Type": "application/json", "Authorization": f"Bearer {resolved['groq_key']}"},
                )
                r.raise_for_status()
                ...
                last_error = e
                continue
        raise last_error or RuntimeError("All pool keys exhausted without a successful response.")
    finally:
        db.close()
...
def ollama_classify_gl_account(
    ...
    content = _groq_chat(GL_SYSTEM_MSG, prompt, temperature, top_p)
...
def ollama_predict_gst(
    ...
    content = _groq_chat(GST_SYSTEM_MSG, prompt, temperature, top_p)
    text = str(content).strip().strip('"').strip("'")
    if text in GST_ENUM:
