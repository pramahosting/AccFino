...
BANK_PRESETS = {
    ...
    # Wise (formerly TransferWise) -- balance statement CSV export.
    # Verified against a real export: columns are "TransferWise ID, Date,
    # Date Time, Amount, Currency, Description, Payment Reference, Running
    # Balance, ...". Single signed Amount column (positive=credit,
    # negative=debit) -- same shape as ANZ/ING, handled by the existing
    # classify_amount() logic, no special-casing needed. Dates are
    # DD-MM-YYYY, already parsed correctly by the dayfirst=True call below
    # (same as every other bank here).
    "Wise": {
        "date":        "Date",
        "description": "Description",
        "amount":      "Amount",
