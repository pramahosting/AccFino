...
const EmptyState = ({ title, detail }) => (
  <div style={{padding:'48px 24px',textAlign:'center',color:'var(--text-3)'}}>
    ...
    <div style={{maxWidth:420,margin:'0 auto',fontSize:'.82rem',lineHeight:1.6}}>{detail}</div>
  </div>
)
const ReportLoading = () => (
  <div style={{padding:48,textAlign:'center',color:'var(--text-3)'}}><span className="spinner"/> Loading…</div>
)

// ── P&L ──────────────────────────────────────────────────────────────────────
function PLReport({ userId }) {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)
  useEffect(() => {
    if (!userId) return
    setLoading(true)
    http.get(`/reports/profit-loss/${userId}`).then(r => setData(r.data)).catch(() => setData(null)).finally(() => setLoading(false))
  }, [userId])

  if (loading) return <ReportLoading/>
  if (!data || !data.has_data) return (
    <EmptyState title="No P&L data yet"
      detail="This report is built from your reconciled transactions' GL accounts (Revenue, Direct Costs, Expense). Reconcile some bank transactions with GL accounts assigned, and this will populate automatically."/>
  )

  const inc = data.income.reduce((s,r)=>s+r.amount,0)
  const cos = data.cost_of_sales.reduce((s,r)=>s+r.amount,0)
  const gross = inc - cos
  const exp = data.expenses.reduce((s,r)=>s+r.amount,0)
  return (
    <div style={{padding:8}}>
      <Sec label="Trading Income" total={inc} accent="#16a34a">{data.income.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>
      <Sec label="Cost of Sales" total={cos} accent="#dc2626">{data.cost_of_sales.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>
      <Tot label="Gross Profit" amount={gross} strong bg/>
      <Sec label="Operating Expenses" total={exp} accent="#d97706" open={false}>{data.expenses.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>
      <Tot label="Net Profit / (Loss)" amount={gross-exp} strong bg/>
    </div>
  )
}

// ── Balance Sheet ─────────────────────────────────────────────────────────────
function BSReport({ userId }) {
  const [data, setData] = useState(null)
  const [loading, setLoading] = useState(true)
  useEffect(() => {
    if (!userId) return
    setLoading(true)
    http.get(`/reports/balance-sheet/${userId}`).then(r => setData(r.data)).catch(() => setData(null)).finally(() => setLoading(false))
  }, [userId])

  if (loading) return <ReportLoading/>
  if (!data || !data.has_data) return (
    <EmptyState title="No Balance Sheet data yet"
      detail="This needs at least one of: bank account balances entered on the reconciliation page, or reconciled transactions posted to Inventory, Fixed Asset, or Equity GL accounts."/>
  )

  const bank = data.bank.reduce((s,r)=>s+r.amount,0)
  const cur = data.current_assets.reduce((s,r)=>s+r.amount,0)
  const fix = data.fixed_assets.reduce((s,r)=>s+r.amount,0)
  const ta = bank + cur + fix
  const te = data.equity.reduce((s,r)=>s+r.amount,0)
  return (
    <div style={{padding:8}}>
      <GH label="Assets"/>
      {data.bank.length > 0 && <Sec label="Bank" total={bank} accent="#2563eb">{data.bank.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>}
      {data.current_assets.length > 0 && <Sec label="Current Assets (Inventory)" total={cur} accent="#0891b2" open={false}>{data.current_assets.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>}
      {data.fixed_assets.length > 0 && <Sec label="Fixed Assets" total={fix} accent="#7c3aed" open={false}>{data.fixed_assets.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>}
      <Tot label="Total Assets" amount={ta} strong bg/>
      {data.equity.length > 0 && (<>
        <GH label="Equity"/>
        <Sec label="Equity" total={te} accent="#16a34a">{data.equity.map((r,i)=><R key={i} account={r.account} amount={r.amount}/>)}</Sec>
...
// ── Executive Summary ─────────────────────────────────────────────────────────
function ExecutiveSummary({ db, userId }) {
  const inc=db?.totals?.credits||0, exp=db?.totals?.debits||0, net=inc-exp, gst=db?.totals?.gst||0, txns=db?.total||0
  const [bs, setBs] = useState(null)
  useEffect(() => {
    if (!userId) return
    http.get(`/reports/balance-sheet/${userId}`).then(r => setBs(r.data)).catch(() => setBs(null))
  }, [userId])
  const bankTotal = bs?.bank?.reduce((s,r)=>s+r.amount,0) ?? null
    ...
    <div style={{padding:20}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        ...
        <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:'var(--r-lg)',overflow:'hidden'}}>
          ...
          <div style={{padding:'0 14px'}}>
            <Row2 l="Bank Accounts" v={bankTotal===null ? 'Not entered yet' : fmtAUD(bankTotal)} s="From account balances you've entered"/>
            <Row2 l="Accounts Receivable" v="Not tracked yet" s="No unpaid-invoice tracking configured"/>
            <Row2 l="Accounts Payable"    v="Not tracked yet" s="No bills/purchases module yet"/>
          </div>
        </div>
      </div>
      <div style={{marginTop:14,padding:'8px 12px',background:'#fef3c7',borderRadius:'var(--r-md)',
        fontSize:'.74rem',color:'#92400e',border:'1px solid #fde68a'}}>
        💡 Income & expense are computed from your reconciliation DB. Bank total comes from account balances you've entered manually or via CSV import.
      </div>
    </div>
  )
}
...
function AgedRec({ userId }) {
  ...
  useEffect(() => {
    if (!userId) return
    setLoading(true)
    http.get(`/reports/aged-receivables/${userId}`)
      .then(r => { setRows(r.data.rows || []); setHasData(r.data.has_data) })
      .catch(() => { setRows([]); setHasData(false) })
  ...
  if (!hasData) return (
    <EmptyState title="No outstanding receivables"
      detail="This report is built from real unpaid invoices (status ≠ paid) in Invoicing. Create some invoices there and this will populate automatically."/>
  )

  const tots=rows.reduce((a,r)=>({cur:a.cur+r.cur,d30:a.d30+r.d30,d60:a.d60+r.d60,d90:a.d90+r.d90,total:a.total+r.total}),{cur:0,d30:0,d60:0,d90:0,total:0})
...
// ── Aged Payables ─────────────────────────────────────────────────────────────
// No bills/purchases model exists anywhere in this app yet -- there is no
// real data source for this report, so it honestly says so instead of
// showing invented supplier names and amounts.
function AgedPay() {
  return (
    <EmptyState title="Aged Payables isn't available yet"
      detail="This app doesn't have a bills/purchases tracking module yet, so there's no real data to build this report from. This will need a supplier-bill entity (with due dates and payment status) before it can show real numbers."/>
  )
}
...
function ReportBody({ rkey, userId, dbStats, hasProReports }) {
  ...
  const bodies = {
    executive_summary:    <ExecutiveSummary db={dbStats} userId={userId}/>,
    pl:                   <PLReport userId={userId}/>,
    balance_sheet:        <BSReport userId={userId}/>,
    aged_receivables:     <AgedRec userId={userId}/>,
